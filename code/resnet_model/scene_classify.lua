...
function parse_args()
   ...
   cmd:option('--path_category', 'data/visual_category/categories_places365.txt', 'Path to categories of places365.')
...
-- load the categories
local cates = {}
for line in io.lines(opts.path_category) do
    table.insert(cates, line:split(' ')[1])
end
...
for k, v in pairs(qs) do
   ...
   for n=1,N do
      if probs[n] >= opts.threshold then
         table.insert(classes, cates[indexes[n]])
      else
         break
      end
...
utils.write_json(opts.dir_output.."/scene.json", ans)
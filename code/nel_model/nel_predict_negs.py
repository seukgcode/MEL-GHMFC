...
def similarity_link(args, query, candidates_list):
    ...
    for i, candidates in enumerate(candidates_list):
        ...
        sim_score, sim_rank = sim.sort(dim=-1, descending=True)
...
def main():
    ...
    texts = [
        "Gabriel with Russian President Vladimir Putin, 9 March 2017",
    ]
    mentions = [
        "Vladimir Putin",
    ]
    ...
    qid2negID = {qid: i for i, qid in enumerate(neg_list)}

    neg_feat_h5 = h5py.File(join(args.dir_neg_feat, "neg_feats.h5"), 'r')
    neg_features = neg_feat_h5.get("features")

    candidate_tensors_list = []
    for cql in candidate_qids_list:
        negIDs_map = [qid2negID[nid] for nid in cql]
    ...
    img_feats_tensor = torch.tensor([img_features[id2img_feat_id[imgID]] for imgID in imgs])
    ...
    rankedQids_list = []
    for i in range(len(linked_ranks)):
        candidate_qids = candidate_qids_list[i]
        ranked_qids = []
